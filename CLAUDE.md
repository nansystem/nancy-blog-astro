...
- デプロイ: Cloudflare の Git 連携で push 時に自動ビルド & `pnpm exec wrangler deploy`
...
| デプロイコマンド            | `pnpm exec wrangler deploy`           | `pnpm exec wrangler pages deploy dist` |
...
| 場所    | パス                                                    | 用途                                  |
| ------- | ------------------------------------------------------- | ------------------------------------- |
| WSL     | `/home/nancy/ghq/github.com/nansystem/nancy-blog-astro` | レイアウト・コード修正、`pnpm dev`    |
...
以下の語句は記事内で使用しない。より具体的な表現に置き換えること。

- **本質** — 「〜の本質は」ではなく、具体的に何を指すのかを書く
- **起点** — 「〜の起点は」ではなく、平易な表現に置き換える
- **核心** — 「〜の核心は」ではなく、具体的に何を指すのかを書く

### 専門用語の説明ルール

記事内で初出の専門用語・業界用語には必ず簡単な説明を付ける。読者が知らない可能性がある用語を説明なしで使わない。

- NG: 「スタックに積む」（スタックの説明なし）
- OK: 「一時退避する」（直感的な言葉を選ぶ）か、用語を使うなら簡単な説明を添える

### 文章スタイルルール

**英語直訳的な表現を避ける（`prh.yml` でも自動検出）**
...
「ファイル1行。〜だ。」のように、短い体言止め・名詞句だけの文を単独で置かない。
文脈を補い、1文として完結させること。

- NG: `設定ファイル1行。あとはCLIが自動でやってくれる。`
- OK: `設定ファイルに1行追加するだけで、CLIが自動で処理する。`
