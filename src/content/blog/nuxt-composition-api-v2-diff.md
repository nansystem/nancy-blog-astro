...
```vue
watch( () => form, (currentForm, prevForm) => { console.info('currentForm: ', currentForm,
'prevForm: ', prevForm) }, { deep: true}, )
```
...
```vue
setup(_, {emit}) { const updateValue = (e) => { emit('input', e.target.value) } return { updateValue
} },
```
