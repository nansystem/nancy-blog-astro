...
「Python File」のようにファイルを指定したり、「Django」や「Flask」のようにフレームワークを指定してデバッグできる。
...
仮引数は複数指定でき、任意でデフォルト値を受け取ることができる。  
関数本体の最初の文としてdocstring(ドキュメンテーション文字列)を書くことができる。  
docstringは任意であり、関数の説明を書いておくとよい。

関数の呼び出しは関数名に続けて`()`で呼び出し、`()`には実引数(関数に渡す値)を指定できる。  
実引数は`仮引数名 = 値`の形で指定でき、これをキーワード引数という。
...
`*名前`の形で書くことで可変長の引数を指定できる。仮引数はタプルとして扱われる。  
...
通常の関数は`def`キーワードから始まり、少なくとも2行以上必要だが、`lambda`キーワードを使うことで関数を1行で定義できる。  
...
また、`@propertyを付与したメソッド名.setter`の形でセッターにできる。
...
用途としては、クラスとしてメソッドを集約できるため、ユーティリティクラスとして使える。
...
`@abstractmethod`をメソッド名の前につけることで、抽象メソッドにでき、継承するクラスにメソッドの実装を強制できる。  
